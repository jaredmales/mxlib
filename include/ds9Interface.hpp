...
public:
   ...
   /** The title is used as the name of the XPA access point
     ...
     * \retval -1 on an error
     */
   int title( const std::string & new_title /**< the title to set. */);
   ...
   /** 
     ...
     * \retval -1 on an error
     */
   int port( const int & new_port /**< the port to set. */);
   ...
   /** Nothing is done if the frame already exists.  Note that this does not open a new frame in ds9.
     ...
     * \retval -1 on an error
     */
   int addsegment(int frame /**< the number of the new frame to initialize.  \note frame must be >= 1. */);
   ...
   /** Nothing is done if the frame already exists.  First calls \ref addsegment.
     ...
     * \retval -1 on an error
     */
   int addframe( int frame /**< the number of the new frame to initialize.  \note frame must be >= 1. */);
   ...
   int display( const dataT *im, ///< the address of the image
                size_t dim1,     ///< the first dimension of the image (in pixels)
                size_t dim2,     ///< the second dimension of the image (in pixels)
                size_t dim3,     ///< the third dimension of the image (in pixels), set to 1 if not a cube.
                int frame = 1    ///< the number of the new frame to initialize.  \note frame must be >= 1.
              );

   template<typename arrayT>
   int display( const arrayT & array, ///< An array containing a image or cube of images
                int frame = 1 ///< the number of the new frame to initialize.  \note frame must be >= 1.
              );
