...
struct turbAtmosphere : public base::changeable<turbAtmosphere<_aoSystemT>>
{
  ...
  protected:
    ...
    std::string m_dataDir; /**< Specifies a path where phase screens are stored.  Leaving this unset or "" is equivalent
  ...
  public:
    ...
    uint32_t maxShift( uint32_t n, realT dt );
    ...
    int shift( improc::milkImage<realT> &phase, realT dt );
...
#pragma omp parallel for
    ...
    {
        phase +=
            sqrt( m_aosys->atm.layer_Cn2( j ) ) * m_layers[j].m_shiftPhase.block( m_buffSz, m_buffSz, m_wfSz, m_wfSz );
    }
    milkPhase.post();
