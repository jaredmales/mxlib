...
struct clGainOpt
{
  ...
  public:
    ...
    int _minFindBits;          ///< The bits of precision to use for minimum finding. Defaults to
    ...
    void b( const Eigen::Array<realT, -1, -1>
                &newB /**< [in] a column-vector Eigen::Array of coefficients, which is copied to _b.*/ );
    ...
    /**
     */
    void a( const Eigen::Array<realT, -1, -1>
                &newA /**< [in] a column-vector Eigen::Array of coefficients, which is copied to _a.*/ );
     ...
     */
    realT optGainOpenLoop(
        realT &var,                         ///< [out] the variance at the optimum gain
...
std::complex<realT> clGainOpt<realT>::olXfer( int fi, complexT &H_dm, complexT &H_del, complexT &H_con )
{
    ...
    if( _changed )
    {
        ...
        for( size_t i = 0; i < _f.size(); ++i )
        {
            ...
            /*if( i == 0 || i == 1)
            {
               std::cerr << i << " " << _f[fi] << " " << s << " " << expsT << " " << _H_wfs[i] << " " << _H_dm[i] << " "
            << _H_con[i] << " " << _H_del[i] << "\n";
               //exit(0);
            }/**/
        }
                       ...
                       uintmax_t &iters )
{
    ...
    try
    {
        ...
        brack = boost::math::tools::brent_find_minima<clGainOptOptGain_OL<realT>, realT>(
            olgo, minFindMin, minFindMaxFact * gmax, minFindBits, minFindMaxIter, iters );
        gopt = brack.first;
        var = brack.second;
    }
    catch( ... )
