...
struct influenceFunctionGaussianSpec
{
     ...
     */
        ...
        for( int i = 0; i < xcoords.size(); ++i )
        {
            ...
            mx::math::func::gaussian2D(
                act_inf.image( i ).data(), pupilSz, pupilSz, 0.0, 1.0, xc, yc, fwhm2sigma( fwhm ) );
        }
...
/** The width of the IFs is specified by the coupling, which is the height of the IF at 1 actuator separation.  For a
 * FWHM = 1 actuator, coupling = 0.0625. The IFs are normalized to a height of 1.
 *
 ...
 * \param [in] coupling is the height of the IF at 1 actuator separation.  E.G., for a FWHM = 1 actuator, set this to
 * 0.0625. \param [in] pupilSz is the size of the pupil, and therefore the size of the maps (pupilSz x pupilSz),  Is set
 * to dmSz if 0, can be pupilSz \< dmSx for an oversized DM.  The pupil is assumed to be centered.
 *
    ...
    bool offsetOdd = false )
{
    ...
    */
    ...
    for( int i = 0; i < xcoords.size(); ++i )
    {
        ...
        mx::math::func::gaussian2D<realT>(
            act_inf.image( i ).data(), pupilSz, pupilSz, 0.0, 1.0, xc, yc, math::func::fwhm2sigma( fw ) );
    }
...
/** The influence functions are a cube of 2D maps of mirror deformations, and the matrix \f$ A \f$ is formed by
 * vectorizing the influence functions. The pseudo inverse is then calculated from the SVD as follows: \f$ A = U S V^T
 * \f$ which gives the Moore-Penrose pseudo-inverse: \f$ A^+ = V S^+ U^T\f$. The columns of U contain the orthogonal
 * mirror modes, and S contains the singular values.  These are both written to disk along with the pseudo-inverse.
 *
 * \param[in] dmName is the name of the deformable mirror.
 * \param[in] maxCondition [optional] the maximum condition number to accept for the pseudo-inverse.  If < 0 [default]
 * then the user is interactively asked what to use.
 *
 * \tparam realT is the real floating point type in which to do calculations.
 */
template <typename realT>
...
/**
  * Given the pseudo-inverse \f$ A^+ \f$ of the influence functions, the commands to the actuators to take a mirror
  shape \f$ \vec{s} \f$ are calculated as
  * \f[
  ...
  * Now given a basis set \f$ \mathbf{M} \f$, the mirror shape is specified as
   ...
   \vec{s}= \sum_i m_i M_i
   \f]
   * where \f$ \vec{m} = m_0, m_1, \cdot m_i \cdot \f$ are the modal coefficients. If the mirror-to-commands matrix, \f$
  \mathbf{M2c} \f$ gives commands as
  * \f[
