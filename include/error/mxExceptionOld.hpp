...
class mxException : public std::exception
{
  ...
  public:
    ...
    mxException( const mxException &e ) noexcept
        : m_source( e.m_source ), m_code( e.m_code ), m_codeName( e.m_codeName ), m_file( e.m_file ),
          m_line( e.m_line ), m_explanation( e.m_explanation )
    {
        build_what();
    }
    ...
    mxException( const std::string &esrc,  ///< [in] the source of the exception, typically the class and function
        ...
        : m_source( esrc ), m_code( ec ), m_codeName( emnem ), m_errno( 0 ), m_file( efile ), m_line( line ),
          m_explanation( expl )
    {
        build_what();
    }
    ...
    mxException( const std::string &esrc,  ///< [in] the source of the exception, typically the class and function
        ...
        : m_source( esrc ), m_code( ec ), m_codeName( emnem ), m_errno( en ), m_file( efile ), m_line( line ),
          m_explanation( expl )
    {
        build_what();
    }
...
class invalidconfig : public mxException
{
  public:
    invalidconfig(
        const std::string &esrc,  ///< [in] the source of the exception, typically the class and function
        const std::string &efile, ///< [in] the source file in which the exception occurred, normally __FILE__
        const int &line,          ///< [in] the line number where the exception was thrown
        const std::string &expl   ///< [in] the explanation for why the exception was thrown
        )
