...
class averagePeriodogram
{
  ...
  protected:
    ...
    size_t m_overlap{ 0 }; ///< The number of samples by which to overlap.  This should almost always be 0.5*m_avgLen.
                           ///< Set 0 for the non-overlapped case.
  ...
  protected:
    ...
    std::vector<realT> m_win; ///< The window function.  By default this is empty, which is equivalent to setting it to
  ...
  public:
    ...
    /** Sets m_overlap = 0.5*m_avgLen.  If you desire the non-overlapped case use
      ...
      * \code
        ...
        \endcode
      */
    explicit averagePeriodogram(
        size_t avgLen, ///< [in] The length of averaging in samples.
        size_t padLen  ///< [in] [optional] the length of the padded sample.  If 0 or omitted then no padding used.
    );

    /// C'tor which sets up the optimum overlapped periodogram of the timeseries with zero-padding and sets the
    /// sampling.
    /** Sets m_overlap = 0.5*m_avgLen.  If you desire the non-overlapped case use
      * the alternate constructor:
      * \code
    ...
    averagePeriodogram( size_t avgLen, ///< [in] The number of samples in each periodgoram estimate.
                        size_t padLen, ///< [in] the length of the padded sample.  Set to 0 for no padding.
                        size_t olap, ///< [in] The number of samples by which to overlap.  This should almost always be
                                     ///< 0.5*m_avgLen.  Set 0 for the non-overlapped case.
                        realT dt     ///< [in] the sampling interval of the time-series
    );
     ...
     *
     */
    void operator()( realT *pgram,    ///< [out] a pre-allocated to size() array which will be populated with the
     ...
     *
     */
    void operator()( std::vector<realT> &pgram,   ///< [out] a vector which will be allocated and populated with the
                                                  ///< periodogram of the time-series
...
template <typename realT>
averagePeriodogram<realT>::averagePeriodogram( size_t avgLen, size_t padLen )
{
...
template <typename realT>
averagePeriodogram<realT>::averagePeriodogram( size_t avgLen, size_t padLen, realT ndt )
{
    resize( avgLen, padLen, 0.5 * avgLen );
    dt( ndt );
}
...
template <typename realT>
averagePeriodogram<realT>::averagePeriodogram( size_t avgLen, size_t padLen, size_t olap, realT ndt )
{
...
template <typename realT>
int averagePeriodogram<realT>::resize( size_t avgLen, size_t padLen, size_t olap )
{
    m_avgLen = avgLen;
    m_padLen = padLen;
...
template <typename realT>
void averagePeriodogram<realT>::operator()( realT *pgram, const realT *ts, size_t sz )
{
    ...
    for( size_t j = 0; j < m_size; ++j )
        pgram[j] *= tsVar / pgramVar;
}

template <typename realT>
void averagePeriodogram<realT>::operator()( std::vector<realT> &pgram, const std::vector<realT> &ts )
{
    pgram.resize( m_size );
