...
public:
   ...
   void doPlan(const meta::trueFalseT<false> & inPlace)
   {
      ...
      _plan = fftw_plan_dft<inputT, outputT>( fftwDimVec<dim>(_szX, _szY, _szZ), forplan1, forplan2,  pdir, FFTW_ESTIMATE);//FFTW_MEASURE);
   ...
   void doPlan(const meta::trueFalseT<true> & inPlace)
   {
      ...
      _plan = fftw_plan_dft<inputT, outputT>( fftwDimVec<dim>(_szX, _szY, _szZ),  reinterpret_cast<inputT*>(forplan), reinterpret_cast<outputT*>(forplan),  pdir, FFTW_ESTIMATE);//FFTW_MEASURE);
