...
/** Derived classes should implement at a minimum
  ...
  *
  * This class uses a cascaded configuration system.  The application configuration is built up from the following
  ...
  * - The command line
  *
  * At each step in the above order, values read from that configuration source override any previous settings. So the
...
class application
{
  ...
  protected:
    ...
    std::string m_configPathGlobal_env; ///< Environment variable to check for the global config path.  Set in
                                        ///< constructor to use.
    std::string m_configPathUser; ///< The path to the user's configuration file.  If the first character is not '/' or
                                  ///< '~', this is added to HOME. Set in constructor to use.
    std::string
        m_configPathUser_env; ///< Environment variable to check fo the user config path.  Set in constructor to use.
    std::string m_configPathLocal; ///< The path to a local configuration file. Set in constructor to use.
    std::string
        m_configPathLocal_env; ///< Environment variable to check for the local config path.  Set in constructor to use.

    bool m_requireConfigPathGlobal{
        true }; ///< Flag controlling whether lack of a global configuration file should be reported.
    bool m_requireConfigPathUser{
        true }; ///< Flag controlling whether lack of a user configuration file should be reported.
    bool m_requireConfigPathLocal{
        true }; ///< Flag controlling whether lack of a local configuration file should be reported.
    ...
    std::string
        m_configPathCLBase_env; ///< Environment variable to check for the CL base path.  Set in constructor to use.
  ...
  protected:
    ...
    /// Setup a basic configuration.  Can be used in an intermediate derived class to allow its children to use
    /// setupConfig.
