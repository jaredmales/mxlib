 ...
 * Eigen-like 2D array type.  Should be resolved by compiler.
 *
 */
...
void rectangleIndices( std::vector<sizeT> &idx, sizeT rows, sizeT cols, sizeT xmin, sizeT xmax, sizeT ymin, sizeT ymax )
{
...
template <class eigenT>
void rectangleIndices( std::vector<size_t> &idx, eigenT &mask, size_t xmin, size_t xmax, size_t ymin, size_t ymax )
{
...
/** The circle is centered at 0.5*(rows-1) and 0.5*(cols-1), and described by its radius. Any value can be set for the
 * mask. Pixels outside the masked circle are not altered.
 *
 * \tparam arrayT is an Eigen-like type.
 *
 * \ingroup image_masks
...
/** The wedge is describe by its center coordinate, central angle, and angular half-width. Any value can be set for the
 * mask. Pixels outside the masked circle are not altered.
 *
 * \tparam arrayT is an Eigen-like type.
 *
...
)
{
    if( fabs( x1 - x0 ) <= 1 && fabs( y1 - y0 ) <= 1 )
    {
        ...
        if( xa >= 0 && xa < mask.rows() && ya >= 0 && ya < mask.cols() )
            mask( (int)xa, (int)ya ) = val;
        return 0;
    }
    ...
    if( fabs( x1 - x0 ) <= 1 )
    {
        ...
        for( realT y = _y0; y <= _y1; y += dy )
        {
            ...
            if( x + 0.5 >= 0 && x + 0.5 < mask.rows() && y + 0.5 >= 0 && y + 0.5 < mask.cols() )
                mask( (int)( x + 0.5 ), (int)( y + 0.5 ) ) = val;
        }
...
)
{
    ...
    if( width < 1 )
        return drawLine( mask, x0, y0, x1, y1, val );
