 ...
 * These functions pad an image by adding rows and columns, and filling in the new pixels with appropriate values.  This
 * can be a constant, or nearest neighbor.  A version is also provided which expands an aribtrary 1/0 mask, which is
 * useful when working with, say, circular regions of an image.
 */
...
int padImage( imOutT &imOut,      ///< [out] On return contains the padded image.  This will be resized.
              ...
              unsigned int padSz, ///< [in] The size of the pad.  The padded image (imOut) will be 2*padSz rows and cols
...
int padImageRef( imOutT imOut,       ///< [out] On return contains the padded image.  This will be resized.
                 imInT imIn,         ///< [in] The image to be padded.
                 unsigned int padSz, ///< [in] The size of the pad.  The padded image (imOut) will be 2*padSz rows and
                                     ///< cols larger than the input.
                 typename imOutT::Scalar value ///< [in] the value to use for padding.
)
{
    int nRows = imIn.rows() + 2 * padSz;
 ...
 * in the mask), the value is chosen as the value of the nearest pixel with a 1 in the mask.  If the closest pixels are
 * more than one equidistant pixels, their mean value is used.
 *
 *
 * \retval 0 on success
 * \retval -1 on error
...
template <typename imOutT, typename imInT>
int cutPaddedImage(
    imOutT &imOut,     ///< [out] On return contains the cut image.  This will be resized.
