...
/** Uses the cubic convolution interpolation kernel.  See <a
 * href="https://en.wikipedia.org/wiki/Bicubic_interpolation">https://en.wikipedia.org/wiki/Bicubic_interpolation </a>.
 *
 * The parameter \ref cubic should be left as the default -0.5 in most cases, which gives the bicubic spline
 * interpolator.  See <a
 * href="https://en.wikipedia.org/wiki/Cubic_Hermite_spline">https://en.wikipedia.org/wiki/Cubic_Hermite_spline</a>.
 *
...
/** The output image can be smaller than the input image, in which case the wrapping (if enabled) still occurs for the
 * input image, but only output images worth of pixels are actually shifted.  This is useful, for instance, when
 * propagating large turbulence phase screens where one only needs a small section at a time.
 *
...
void imageShiftWP( outputArrT &out, ///< [out] contains the shifted image.  Must be pre-allocated, but can be smaller
                                    ///< than the in array.
...
/** The output image can be smaller than the input image, in which case the wrapping still occurs for the input image,
 * but only output images worth of pixels are actually shifted.  This is useful, for instance, when propagating large
 * turbulence phase screens where one only needs a small section at a time.
 *
...
)
{
    ...
    if( dx == floor( dx ) && dy == floor( dy ) )
        return imageShiftWP( transim, im, dx, dy, false );
...
/** Uses the given transformation type to magnify the input image to the size of the output image.
  ...
  * \endcode
  * Some care is necessary to prevent magnification from shifting the image with respect to this center.  The main
  result is that the
...
int imageRebinSum( imageOutT &imout,     ///< [out] the re-binned image.  Must be allocated to size which is an integer
                                         ///< factor smaller than imin.
...
/// Re-bin an image using the sum, reducing its size while conserving the total flux.  Records the value and position of
/// the re-binned max pixel.
/** Optionally this can be the mean instead of the sum filter, in which case total flux is not conserved.
 *
 * \overload
 */
 ...
 */
template <typename imageOutT, typename imageInT>
int imageRebinMean( imageOutT &imout,    ///< [out] the re-binned image.  Must be allocated to size which is an integer
                                         ///< factor smaller than imin.
                    const imageInT &imin ///< [in] the image to rebin
)
{
 ...
 */
template <typename imageOutT, typename imageInT>
int imageRebinMeanReject( imageOutT &imout,    ///< [out] the re-binned image.  Must be allocated to size which is an
                                               ///< integer factor smaller than imin.
                          const imageInT &imin ///< [in] the image to rebin
)
{
    int rebin = imin.rows() / imout.rows();
        ...
        for( int j = 0; j < imout.cols(); ++j )
        {
            ...
            for( int k = 0; k < rebin; ++k )
            {
                for( int l = 0; l < rebin; ++l )
                {
                    ...
                    if( imin( i * rebin + k, j * rebin + l ) > max )
                        max = imin( i * rebin + k, j * rebin + l );
                    if( imin( i * rebin + k, j * rebin + l ) < min )
                        min = imin( i * rebin + k, j * rebin + l );
                }
            }
...
/// Re-bin an image, takes the mean with a min/max rejection.  Records the value and position of the re-binned max
/// pixel.
/** The mean is calculated after rejecting the minimuma and maximum value.
 *
 * \overload
