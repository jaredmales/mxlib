...
protected:
   ...
   realT m_xcen {0}; ///< The x coordinate of the center of the image.  Radii are caculated relative to this point. Default is 0.5*(sizeX-1).
   realT m_ycen {0}; ///< The y coordinate of the center of the image.  Radii are caculated relative to this point. Default is 0.5*(sizeY-1).
...
public:
   ...
   int resize( int sizeX,  ///< [in] The new size in rows
               ...
               realT xcen, ///< The x coordinate of the center of the image.  Radii are caculated relative to this point. Default is 0.5*(sizeX-1).
               realT ycen  ///< The y coordinate of the center of the image.  Radii are caculated relative to this point. Default is 0.5*(sizeY-1).
             );
   ...
   /** 
     ...
     */
   int cumPhot( std::vector<realT> & cumPhot, ///< [out] the cumulative photometry at each point in the radius vector.  Resized.
                std::vector<realT> & deltaPhot, ///< [out] [optional] the photometry at each point in the radius vector, not-cumulative.  Resized.
                eigenImage<realT> & im,       ///< [in] the image on which to perform the calculation
                realT maxr = 0                ///< [in] [optional] the maximum radius to which to calculate.  If <= 0 then max possible is used.
              );
...
protected:
   ...
   /** This is called by the public cumPhot functions.
     ...
     * \returns -1 on error
     */
   int cumPhotWork( std::vector<realT> & cumPhot,   ///< [out] the cumulative photometry at each point in the radius vector.  Resized.
...
template<typename realT>
int aperturePhotometer<realT>::resize( int sizeX,  
                                       int sizeY,  
                                       realT xcen, 
                                       realT ycen  
                                     )
{
   ...
   if( sizeX == m_sizeX && sizeY == m_sizeY && xcen == m_xcen && ycen == m_ycen) return 0;
