...
class sourceFinder
{
  ...
  protected:
    ...
    bool m_useMedian{
        true }; ///< If true, then the median is used for standard deviation calculation. The mean is used otherwise.
...
template <typename realT>
sourceFinder<realT>::sourceFinder( realT thresh, realT minsep ) : m_threshold( thresh ), m_minsep( minsep )
{
}
...
std::vector<typename sourceFinder<realT>::pixelT> sourceFinder<realT>::operator()( const eigenImage<realT> &im )
{
    ...
    if( m_useMedian )
        mm = math::vectorMedian( m_vals );
    else
        mm = math::vectorMean( m_vals );
    ...
    for( long n = 0; n < m_snr.size(); ++n )
    {
        for( long m = 0; m < m_snr.size(); ++m )
        {
            ...
            // Squared distance:
            realT d2 = pow( m_pixs[m_snr[n]].first - m_pixs[m_snr[m]].first, 2 ) +
                       pow( m_pixs[m_snr[n]].second - m_pixs[m_snr[m]].second, 2 );
