...
template <typename typeT>
void textTable::addCell( size_t row, size_t col, const typeT &cell, int precision )
{
...
void textTable::outPut( iosT &ios )
{
    ...
    for( size_t i = 0; i < m_rows.size(); ++i )
    {
        ...
        {
            if( m_rows[i][j].size() > rowL )
                rowL = m_rows[i][j].size();
        }
        ...
        for( size_t k = 0; k < rowL; ++k )
        {
            ...
            for( size_t j = 0; j < m_colWidths.size(); ++j )
            {
                if( m_rows[i][j].size() > k )
                    line.replace( startPos, m_rows[i][j][k].length(), m_rows[i][j][k] );
                startPos += m_colWidths[j];

                if( j < m_colWidths.size() - 1 )
                    line.replace( startPos, m_colSep.length(), m_colSep );
        ...
        if( m_rowSep.length() > 0 )
            ios << m_rowSep << "\n";
    }
}

extern template void textTable::outPut<std::ostream>( std::ostream &ios );
