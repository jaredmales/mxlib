...
SCENARIO( "creating 2D Rectangular Tukey Windows", "[sigproc::signalWindows::tukey2dSquare]" ) 
{
    GIVEN("a centered square array")
    {
        WHEN("256x256, alpha=0")
        {
            ...
            mx::sigproc::window::tukey2dSquare<float>( win.data(), win.rows(), win.cols(), 256,256, 0.0, 0.5*(win.rows()-1.0), 0.5*(win.cols()-1.0));
        ...
        WHEN("256x256, alpha=1")
        {
            ...
            mx::sigproc::window::tukey2dSquare<float>( win.data(), win.rows(), win.cols(), 256,256, 1.0, 0.5*(win.rows()-1.0), 0.5*(win.cols()-1.0));
        ...
        WHEN("256x256, alpha=0.5")
        {
            ...
            mx::sigproc::window::tukey2dSquare<float>( win.data(), win.rows(), win.cols(), 256,256, 0.5, 0.5*(win.rows()-1.0), 0.5*(win.cols()-1.0));
