    ...
    void close() override
    {
        ...
        const bool should_wait_for_child = [&]()
        {
            ...
            if( std::find_if( m_children.begin(),
                              ...
                              { return tracker->hasStarted(); } ) != m_children.end() )
            {
                return false;
            }
...
{
    return std::all_of(
        m_patterns.begin(), m_patterns.end(), [&]( PatternPtr const &p ) { return p->matches( testCase ); } );
}
...
#define CATCH_SUCCEED( ... )                                                                                           \
    INTERNAL_CATCH_MSG(                                                                                                \
        "CATCH_SUCCEED", Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure, __VA_ARGS__ )
